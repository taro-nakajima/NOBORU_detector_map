...
// bank number 0:SM16, 1:ML, 2:HL
const BankHorAngleMin = [0.0189019157990986, 0.407221034, 0.765013531, -0.544643747]; // angles of the right edge of the detector banks (rad) 
const BankHorAngleMax = [0.179768912955416, 0.407221034, 0.765013531, -0.544643747]; // angles of the right edge of the detector banks (rad) 
//const BankAngleMax = [0.179883103929578, 0.736291421, 1.094083918, -0.215347289]; // angles of the left edge of the detector banks (rad)
const Coefphihvsdet0 = [0.0185578521822071, -0.577419264, -0.548697154, -2.188553408];    // 0th-order coefficients for  phih vs detector 
const Coefphihvsdet1 = [0.000177006198945301,0,0,0]; // 1st-order coefficients for phih vs detector, the same value for all the banks except the direct-beam bank  

const Hpixel_ofst = [450,0,0,0];
const Hpixel_sign = [-1,0,0,0];

const BankWidth = [910,300,400];    // in mm unit
const BankHeight = [1000,1630,1361.5];    // in mm unit

const DetMin = 280; // minimum of detector number for drawing 
const DetMax = 392; // maximum of detector number for drawing plus 1
...
const DetBankScale = 0.5;   // convert mm to pixel.
...
function draw() {
    ...
    showUBmatrix();
    TOF_lim_adjust_and_draw();
    draw_OriViewer();

}

function set_Inst_and_draw(){
    set_Instrument();
    draw_DetMap();
}
...
function TOF_lim_adjust_and_draw(){
    document.getElementById("TOF_max_disp").value = document.getElementById("TOF_max").value;
    TOF_max = Number(document.getElementById("TOF_max").value);
    document.getElementById("TOF_min_disp").value = document.getElementById("TOF_min").value;
    TOF_min = Number(document.getElementById("TOF_min").value);
    draw_DetMap();
}
...
function set_Instrument(){
    L_1=Number(document.getElementById("L1").value);
    L2=Number(document.getElementById("L2").value);
    det_ofst_hor=Number(document.getElementById("det_ofst_hor").value);
    det_ofst_ver=Number(document.getElementById("det_ofst_ver").value);
...
function draw_DetMap(){
    ...
    let Ei_max = ((L_1+L2)/1000.0*TOFconst/(TOF_min/1000.0))**2.0;
...
function drawBraggReflection(context1,H1,K1,L1,isTargetHKL1,showHKL1){
    ...
    if(Ghkl[0]>=0.0){
        ...
        let TOF_hkl = (L_1+L2)/1000.0*2.286/Math.sqrt(Ei_hkl)*1000.0;

        if(TOF_hkl<TOF_max && TOF_hkl>TOF_min && G_sq > 0){   // lambda_min=2PI/sqrt(Ei_max/2.072), the case that H=K=L=0 is avoided by the condition of  G_sq > 0.
            ...
            if(phih<0){
                let PosX=canvas.width/2 + (L2*Math.sqrt(1.0+Math.tan(phih+Math.PI/2.0)**2.0)*Math.tan(phiv)+det_ofst_ver)/(detSize/2.0)*(canvas.width/2.0);
                let PosY=canvas.height/2 + (L2*Math.tan(phih+Math.PI/2.0)+det_ofst_hor)/(detSize/2.0)*(canvas.height/2.0);
                ...
                if(PosX>=0 && PosX<canvas.width && PosY >= 0 && PosY <=canvas.height){
                    ...
                    if(isTargetHKL1==true){
                        ...
                        document.getElementById("TOF").innerHTML=Math.round(TOF_hkl*decimal_digit)/decimal_digit;        
...
/*    let geometry = new Array(BankAngleMin.length);
    ...
    geometry_det = new THREE.BoxGeometry(256*DetBankScale, 256*DetBankScale,DetBankThickness);
    mesh_det = new THREE.Mesh(geometry_det, material1);
    scene.add(mesh_det);
//    mesh_det.rotation.y += Math.PI;       //rotation about the y axis.
    //mesh_det.position.x += LB20*DetBankScale*Math.cos((BankAngleMin[i]+BankAngleMax[i])/2.0);    // move along the x axis.
    mesh_det.position.z += 530*DetBankScale;    // move along the z axis.    
...
function phih2hpixel(phih){    // convert phih to detector number
    let hpixel=-1;
    for(let i=0;i<BankHorAngleMin.length; i++){
        if((phih >= BankHorAngleMin[i]) && (phih <= BankHorAngleMax[i])){
            const dLength = (phih-Coefphihvsdet0[i])/Coefphihvsdet1[i];
            hpixel = Hpixel_ofst[i]+Hpixel_sign[i]*dLength*mm2Xpixel;
        }
    }
    return hpixel; 
}
...
function calcL20(phi_h){      // calculate L20 
    return Math.sqrt(1+Math.tan(phi_h)**2.0)*5586.000000;
}
